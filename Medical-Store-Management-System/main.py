...
def open_win():
    ...
    flag = 'apt'
    apt = Tk()
    apt.title("Medical Store")
    Label(apt, text="GEC MEDICAL STORE COMPANY").grid(row=0, column=0, columnspan=3)
    Label(apt, text='*' * 110).grid(row=1, column=0, columnspan=4)
    Label(apt, text='-' * 110).grid(row=3, column=0, columnspan=4)
    ...
    Button(apt, text='Add and View Products', bg='green', fg='white', width=25, command=stock).grid(row=5, column=0)
    ...
    Button(apt, text='Modify Products', width=25, bg='blue', fg='white', command=modify).grid(row=4, column=1)
    Button(apt, text='Search For Medicine', width=25, bg='blue', fg='white', command=search).grid(row=5, column=1)
    Button(apt, text='Expiry Check', bg='red', fg='white', width=25, command=exp_date).grid(row=6, column=1)

    Label(apt, text="Handle Cash Flows", bg='skyblue', fg='black').grid(row=2, column=2)
    Button(apt, text="Check Today's Revenue", bg='skyblue', fg='black', width=25, command=show_rev).grid(row=5,
                                                                                                         column=2)
    Button(apt, text='Sales Invoice', width=25, bg='skyblue', fg='black', command=billing).grid(row=4, column=2)
    Button(apt, text='Logout', bg='red', fg='white', width=25, command=again).grid(row=6, column=2)
    apt.mainloop()
...
def stock():
    ...
    accept = [''] * 10
    sto = Tk()
    sto.title('STOCK ENTRY AND VIEW PRODUCT')
...
def make_bill():
    ...
    for k in range(len(sl)):
        ...
        for i in cur:
            price[k] = float(qty[k]) * float(i[4])
            print(qty[k], price[k])
            cur.execute("update med set qty_left=? where sl_no=?", (int(i[3]) - float(qty[k]), sl[k]))
        c.commit()
    ...
    m += "====================================================\n"
    m += "                                  No :%s\n\n" % det[5]
    m += " GEC MEDICAL STORE COMPANY\n"
    m += " BBSR BRANCH, BHUBANESWAR\n\n"
    m += "----------------------------------------------------\n"
    if t == 1:
        m += "Name: %s\n" % named
        m += "Address: %s\n" % addd
    ...
    if t == 1:
        ...
        m += "Valued customer Discount" + (' ' * (20 - len(str(round((total - ntotal), 2))))) + '-' + str(round((total - ntotal), 2)) + '\n'
        m += "----------------------------------------------------\n"
        m += 'Total' + (' ' * 25) + (' ' * (12 - len(str(round(ntotal, 2))))) + 'PHP ' + str(round(ntotal, 2)) + '\n'
        det[3] = str(ntotal)
    else:
        m += 'Total' + (' ' * 28) + (' ' * (12 - len(str(total)))) + 'PHP ' + str(total) + '\n'
        det[3] = str(total)

    m += "----------------------------------------------------\n\n"
    m += "Dealer 's signature:________________________________\n"
    m += "====================================================\n"
    print(m)
    p = time.localtime()
...
def show_rev():
    ...
    for i in cur:
        if i[4] == today:
            total += float(i[3])
    print(total)
    Label(rev, width=22, text='Total revenue: PHP ' + str(round(total, 2)), bg='blue', fg='white').grid(row=1, column=0)
    cx = 0
    ...
    for i in cur:
        if i[4] == today:
            cx += 1
            lb1.insert(cx, 'Bill No.: ' + str(i[5]) + '    : PHP ' + str(round(i[3], 2)))
    c.commit()
...
def again():
    ...
    root.title('MEDICAL STORE COMPANY')
    Label(root, text='GEC MEDICAL STORE COMPANY').grid(row=0, column=0, columnspan=5)
    Label(root, text="BBSR BRANCH, BHUBANESWAR").grid(row=1, column=0, columnspan=5)
    ...
    Button(root, width=8, bg='blue', fg='white', text='Enter', command=check).grid(row=5, column=1)
    Button(root, width=6, bg='red', fg='white', text='Close', command=root.destroy).grid(row=5, column=0)
    root.mainloop()
...
def open_cus():
    ...
    apt = Tk()
    apt.title("Med. Store")
    Label(apt, text="*** GEC MEDICAL DRUG STORE ***", bg='blue', fg='white').grid(row=0, column=0)
    Label(apt, text='*' * 45).grid(row=1, column=0)
    Label(apt, text='*  WELCOME CUSTOMER SERVICES  *', bg='green', fg='white').grid(row=2, column=0)
    Label(apt, text='-' * 45).grid(row=3, column=0)

    Label(apt, text='-' * 45).grid(row=5, column=0)
    ...
    Label(apt, text='-' * 45).grid(row=8, column=0)
